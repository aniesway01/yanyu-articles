...
# ── API Keys (from APIkeyBase.md, regenerated 2026-02-28) ──
# Testing Project (3 keys) + BuyService Project (3 keys) + FreeProject (1 key)
# Quota: PER PROJECT (not per key). Pro image: 50 RPD per project (Free Tier)
GEMINI_KEYS = [
    # Testing Project (gen-lang-client-0890171044)
    "AIzaSyA2iBlpgmNFqHE6dmMI25W6R_g8zzKgB0E",
    "AIzaSyCfqvbX-wsxPrIjrf1oSs8Lzf1sGftqJYE",
    "AIzaSyDerapvQfhCRFzUpclUPyT2nX-4ZFstBV0",
    # BuyService Project (gen-lang-client-0637053763)
    "AIzaSyBYvB2oexgLl07FSqeC-A4tAaJ4OVdTxAM",
    "AIzaSyDYjqzI4kRAtU3G8-C9BKcQFKjabgs1bME",
    "AIzaSyBu5xGN5cbNk6_6QgkO-MS_Pit9UnLuAoY",
    # FreeProject (gen-lang-client-0835688722) — disabled: daily quota exhausted 2026-02-28
    # "AIzaSyA84EgMv6B6-gX-x2NskP3GCDnTHNMtakI",
]
...
class KeyRotator:
    """Project-aware round-robin: rotates across PROJECTS (not just keys).

    Gemini Free Tier quota is PER PROJECT:
      - Pro image: 2 RPM, 50 RPD per project
    Keys in the same project share the same quota.
    """

    # Map each key to its project for project-level throttling
    PROJECT_MAP = {
        "AIzaSyA2iBlpgmNFqHE6dmMI25W6R_g8zzKgB0E": "testing",
        "AIzaSyCfqvbX-wsxPrIjrf1oSs8Lzf1sGftqJYE": "testing",
        "AIzaSyDerapvQfhCRFzUpclUPyT2nX-4ZFstBV0": "testing",
        "AIzaSyBYvB2oexgLl07FSqeC-A4tAaJ4OVdTxAM": "buyservice",
        "AIzaSyDYjqzI4kRAtU3G8-C9BKcQFKjabgs1bME": "buyservice",
        "AIzaSyBu5xGN5cbNk6_6QgkO-MS_Pit9UnLuAoY": "buyservice",
        "AIzaSyA84EgMv6B6-gX-x2NskP3GCDnTHNMtakI": "freeproject",
    }

    def __init__(self, keys, rpm_per_project=2):
        # Deduplicate: pick one key per project
        seen_projects = {}
        self.project_keys = []  # [(project_name, key), ...]
        for k in keys:
            proj = self.PROJECT_MAP.get(k, k[-8:])
            if proj not in seen_projects:
                seen_projects[proj] = k
                self.project_keys.append((proj, k))
        self.rpm = rpm_per_project
        self.idx = 0
        self.call_times = {proj: [] for proj, _ in self.project_keys}
        log.info(f"KeyRotator: {len(self.project_keys)} projects: {[p for p,_ in self.project_keys]}")

    def get_client(self):
        from google import genai
        proj, key = self.project_keys[self.idx]
        self.idx = (self.idx + 1) % len(self.project_keys)
        return genai.Client(api_key=key), key

    def wait_if_needed(self, key):
        """Enforce per-PROJECT RPM limit (not per-key)."""
        proj = self.PROJECT_MAP.get(key, key[-8:])
        now = time.time()
        times = self.call_times.get(proj, [])
        # Remove entries older than 60s
        self.call_times[proj] = [t for t in times if now - t < 60]
        times = self.call_times[proj]
        if len(times) >= self.rpm:
            wait = 60 - (now - times[0]) + 2
            if wait > 0:
                log.info(f"  (project {proj} RPM limit, wait {wait:.0f}s)")
                time.sleep(wait)
        self.call_times[proj].append(time.time())
